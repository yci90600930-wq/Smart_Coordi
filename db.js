...
const DB_VERSION=1;
export const STORES=['companies','projects','visits','recordings','issues','causes','improvements','kpis','hardwares','softwares','tasks'];
let dbPromise;
export function db(){
...
export async function put(store,obj){const d=await db();return new Promise((res,rej)=>{const tx=d.transaction(store,'readwrite');const os=tx.objectStore(store);const payload={...obj,updated_at:new Date().toISOString()}; if(!payload.id) delete payload.id; const r=os.put(payload);r.onsuccess=()=>res(r.result);r.onerror=()=>rej(r.error);});}
...
export async function exportData(){const out={version:1,exported_at:new Date().toISOString(),stores:{}};for(const s of STORES){if(s==='recordings'){const rows=await all(s);out.stores[s]=rows.map(({blob,...rest})=>rest);}else out.stores[s]=await all(s);}return out;}
